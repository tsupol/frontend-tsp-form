...
- **Answering BE work orders (DONE files)** — `.claude/done-file-workflow.md`. Test runs go in an `outbox/` test log as they happen; "done" generates the DONE draft from git + latest green tests; "ส่ง BE" verifies, ships to nnf `UI_FEEDBACK/`, and cleans up. DONE files answer only what BE asked (what/which user/blockers) — no FE internals. That flow is the standing permission for DONE responses; other `UI_FEEDBACK/` docs still need the explicit yes below.
