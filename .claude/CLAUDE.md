...
- **Deploying or SSHing to the server** — read `.claude/server-and-deploy.md` first. `just deploy` does not work on this box.
...
- **Answering BE work orders (DONE files)** — `.claude/done-file-workflow.md`. Test runs go in an `outbox/` test log as they happen (the orchestrating session creates the log BEFORE coding or spawning agents; subagent briefs carry the doc's §Subagent-briefs paragraph); "done" / "ส่ง BE" → invoke the `done` skill, which drafts from git + latest green tests, then verifies, ships to nnf `UI_FEEDBACK/`, and cleans up. A SessionStart hook injects `.claude/session-start-drill.md` as the standing reminder. DONE files answer only what BE asked (what/which user/blockers) — no FE internals. That flow is the standing permission for DONE responses; other `UI_FEEDBACK/` docs still need the explicit yes below.
