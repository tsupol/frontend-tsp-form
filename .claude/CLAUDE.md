...
- **In-app printing (bill receipt / sticker / signing detail)** — see `.claude/print-pattern.md` before adding any `window.print()` flow. Browser-print via off-screen body portal + `body:has(.marker)` `@media print` isolation + two RAFs. NOT the server-PDF path (`ContractPreviewModal`/`PdfCanvasViewer`). Reinvented and shipped broken before — read the three GOTCHAs.
...
- **Surface backend gaps to BE, don't paper over them.** When a view/RPC errors in a way that's clearly a backend mistake (missing GRANT, security_invoker view joining an ungranted ref table, RPC param drift, RLS misconfig), file `D:/dev/nnf/UI_FEEDBACK/YYYY-MM-DD_ALERT_<topic>.md` with repro + root-cause guess + suggested SQL fix, AND build the FE to render a visible warning row pointing at that doc (e.g. `alert alert-warning` with the filename) instead of silently swallowing the error. BE reads these; a silent fallback hides the bug from them.
