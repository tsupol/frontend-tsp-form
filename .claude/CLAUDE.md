...
- **NEVER create or commit a `UI_FEEDBACK/` doc (ASK/ALERT/DELIVERY/etc.) without explicit permission.** Filing into the nnf repo is outward-facing — BE reads and acts on these. Always ask first and get a yes before writing the file. When you think a backend gap is worth surfacing, describe it in chat and propose the doc; let the user decide whether to file. This applies even when another rule (e.g. "surface backend gaps") seems to call for filing — the ask-first gate overrides it. If you already created/committed one without asking, revert it.
- **Surface backend gaps to BE, don't paper over them — but ask before filing.** When a view/RPC errors in a way that's clearly a backend mistake (missing GRANT, security_invoker view joining an ungranted ref table, RPC param drift, RLS misconfig), propose a `D:/dev/nnf/UI_FEEDBACK/YYYY-MM-DD_ALERT_<topic>.md` (repro + root-cause guess + suggested SQL fix) and ask for permission to file it; meanwhile build the FE to render a visible warning row pointing at the issue (e.g. `alert alert-warning`) instead of silently swallowing the error. A silent fallback hides the bug from BE.
