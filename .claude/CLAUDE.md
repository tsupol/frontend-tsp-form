...
- **Image upload / drag-drop** — read `.claude/media-upload-pattern.md` before writing ANY upload UI. **Never hand-roll a drop zone**: `ImageUploader` from tsp-form already does click + drag in one element, and supports multi-image via `multiple` + `maxFiles`. A custom zone shipped here once that was drop-only and couldn't be clicked. If you're typing `onDrop`/`dataTransfer`, you're rebuilding the component. Also covers HEIC intake (`convertHeicToJpeg`, Safari-only decode).
