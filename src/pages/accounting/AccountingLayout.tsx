...
export function AccountingLayout({ children }: { children: ReactNode }) {
  ...
  const navItems: NavItem[] = useMemo(() => [
    ...
    { type: 'group', labelKey: 'nav.groupCheckItems' },
    { type: 'link', path: '/admin/accounting/payments', labelKey: 'nav.payments', icon: Banknote },
    { type: 'link', path: '/admin/accounting/installment-check', labelKey: 'nav.installmentCheck', icon: ListChecks },
    { type: 'group', labelKey: 'nav.groupRemitDayClose' },
    ...
    { type: 'link', path: '/admin/accounting/commission-branch', labelKey: 'nav.commissionBranch', icon: Store },
    ...(canSeeFinancier ? [
  ...
  return (
    <div className="flex h-dvh overflow-hidden">
      <nav className="hidden lg:flex flex-col gap-1 shrink-0 w-48 border-r border-line p-4 pt-7.5 h-full overflow-y-auto better-scroll">
        {navItems.map((item, i) => {
          if (item.type === 'group') {
            return (
