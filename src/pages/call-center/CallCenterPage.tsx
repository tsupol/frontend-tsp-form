...
import { ArrowLeft, ArrowRightFromLine, XCircle, Plus, Check, Star } from 'lucide-react';
import { apiClient } from '../../lib/api';
import { DateTime } from '../../components/DateTime';
import { fmtCurrency } from '../../lib/format';
import {
  ccKeys, useFlagLevels, focusAdd, focusRemove, overdueColor, type BookRow,
} from './callCenterApi';
import { FlagPair, SkipReasonBadge } from './ccBadges';
import { ContractDunningDetail } from './ContractDunningDetail';

type View = 'focus' | 'book';
type SortKey =
  | 'work_priority.desc'
  | 'overdue_days.desc'
  | 'overdue_amount.desc'
    ...
    <PageNav panels={['list', 'detail']} defaultPanel={selectedId ? 'detail' : undefined} className="h-dvh">
      {({ isMobile, isRoot, goTo, goBack }) => (
        <>
          ...
          {(isRoot || !isMobile) && (
            <div className="flex-none px-4 py-2 border-b border-line flex flex-col gap-2">
              {/* View toggle */}
              <div className="flex items-center gap-1">
                <Button
                  variant={view === 'focus' ? 'solid' : 'ghost'}
                  color="primary"
                  size="sm"
                  startIcon={<Star size={14} />}
                  onClick={() => { setView('focus'); setPageIndex(0); }}
                >
                  {t('callCenter.myFocus')}{focusCount > 0 ? ` (${focusCount})` : ''}
                </Button>
                <Button
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                          ...
                          <button
                            type="button"
                            className="ml-auto shrink-0 flex items-center justify-center w-6 h-6 rounded bg-transparent border-none cursor-pointer text-subtle hover:text-primary-fg hover:bg-surface-hover"
                            title={c.on_focus ? t('callCenter.removeFromFocus') : t('callCenter.addToFocus')}
                            onClick={(e) => toggleFocus(c, e)}
                          >
                            {c.on_focus ? <Check size={15} className="text-primary-fg" /> : <Plus size={15} />}
                          </button>
                        </div>
                      </div>
                    );
