...
export function PanelProductPlan({ onClose }: Props) {
  ...
  const { data: brands = [] } = useQuery({
    ...
    queryFn: () => apiClient.get<BrandLookup[]>(`/v_ref_brand_list?holding_id=eq.${holdingId}&is_active=is.true&order=name`),
    staleTime: 5 * 60 * 1000,
  });

  const { data: families = [] } = useQuery({
    queryKey: ['family-lookup', holdingId],
    queryFn: () => apiClient.get<FamilyLookup[]>(`/v_ref_product_family_list?holding_id=eq.${holdingId}&is_active=is.true&order=display_name`),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const variants = useMemo((): Variant[] => {
    ...
    for (const q of quoteData.quotes) {
      if (!seen.has(q.variant_id)) seen.set(q.variant_id, { variant_id: q.variant_id, item_name: q.item_name });
    }
        ...
        <div className="border border-line rounded-lg overflow-hidden max-h-48 overflow-y-auto better-scroll">
          {modelsLoading ? (
            ...
            <div className="flex flex-col divide-y divide-line">
              {(models ?? []).map(model => (
                <button key={model.model_id} className={`w-full text-left px-4 py-2.5 flex items-center gap-3 cursor-pointer transition-colors ${model.model_id === localModelId ? 'bg-primary/10' : 'hover:bg-surface-hover'}`} onClick={() => handleSelectModel(model)}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {variants.map(variant => (
                  <button key={variant.variant_id} className={`text-left px-3 py-2.5 rounded-lg border transition-colors cursor-pointer ${localVariantId === variant.variant_id ? 'border-primary bg-primary/5' : 'border-line hover:border-fg/30'}`} onClick={() => handleSelectVariant(variant)}>
...
function QuoteTable({ rows, terms, isSelected, onSelect, t, fmt }: {
  rows: Quote[]; terms: number[];
  isSelected: (q: Quote) => boolean; onSelect: (q: Quote) => void;
            ...
            return termRows.map((row, i) => {
              ...
              return (
                <tr key={`${term}-${row.down_percent}`} className={`cursor-pointer transition-colors ${selected ? 'bg-primary/10' : 'hover:bg-surface-hover'}`} onClick={() => onSelect(row)}>
                  <td className="px-2 py-2.5 text-center">{selected && <Check size={14} className="text-primary inline" />}</td>
