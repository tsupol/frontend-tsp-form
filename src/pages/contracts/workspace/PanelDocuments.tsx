...
import { XCircle, CreditCard, Info, Eye, ScrollText, AlertTriangle } from 'lucide-react';
...
export function PanelDocuments({ onClose: _onClose }: Props) {
  ...
  return (
    <div className="p-4 flex flex-col gap-8 max-w-2xl">
      ...
      {/* Contract preview — customer reviews the rendered contract before signing */}
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          <ScrollText size={14} />
          <label className="form-label mb-0">{t('workspace.docPreviewContract')}</label>
        </div>
        <div className="alert alert-info">
          <Info size={14} />
          <div className="alert-description">{t('workspace.docPreviewHint')}</div>
        </div>
        <div>
          <Button
            color="primary"
      ...
      {/* Signature — captured after the customer has reviewed the preview */}
      <SignatureCapture
