...
interface Props {
  /** Page title (mobile header, desktop heading). */
  title: string;
  /** URL prefix for selection deep-link, e.g. '/admin/contracts/search'. */
  routePrefix: string;
  /** State filter passed to fn_contract_search.p_states. null = all states. */
  states: string[] | null;
  /** Optional slot above the keyword filter row — e.g. scope tabs. */
  headerSlot?: ReactNode;
  /** Hide the state filter pill in the expandable filters row (when scope tabs cover it). */
  showStateFilter?: boolean;
  /** Optional "New contract" button in the desktop header. */
  showNewButton?: boolean;
  /** Override the right-hand cell in each list row (default: installment amount + paid/total). */
  renderRowRight?: (contract: ContractSearchResult) => ReactNode;
  /** Empty-state label override. */
  emptyMessage?: string;
  /** Fires whenever the visible contracts change — lets pages do their own
   *  per-row enrichment queries (e.g. fetch open bills for pending-payment). */
  onContractsChange?: (contracts: ContractSearchResult[]) => void;
}
...
export function ContractListPane({
  ...
  const setSelectedId = (id: number | null) => {
    if (id) navigate(`${routePrefix}/${id}`, { replace: true });
  ...
  const { data: searchData, isFetching } = useQuery({
    queryKey: ['contract-search', states, debouncedKeyword, filterState, filterBranchId, page, pageSize, isMultiWord],
    queryFn: async (): Promise<SearchResponse> => {
      // filterState (if set) narrows further within the pane's fixed `states`.
      // If the pane has fixed states, the picked filter must be one of them.
      const p_states = filterState ? [filterState] : states;
  ...
  const defaultRowRight = (contract: ContractSearchResult) => {
    if ((contract.state === 'DRAFT' || contract.state === 'SAVING') && (contract.total_paid ?? 0) > 0) {
      return (
  ...
  const rowRight = renderRowRight ?? defaultRowRight;
        ...
        <>
          {isMobile && (
            <MobileHeader className="mobile-header-bordered">
              ...
              <div className="mobile-header-title mobile-header-title-truncate">
                {isRoot ? title : (contracts.find(c => c.id === selectedId)?.code_display ?? contracts.find(c => c.id === selectedId)?.code ?? '')}
              </div>
          ...
          {!isMobile && (
            <div className="flex-none px-4 py-2.5 border-b border-line flex items-center gap-4">
              <h1 className="heading-2 shrink-0">{title}</h1>
              {showNewButton && (
